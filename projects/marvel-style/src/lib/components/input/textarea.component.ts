...
@Component({
  selector: 'marvel-textarea',
  template: `
    <mc-form-field [ngClass]="{ disabled: _disabled }">
      <mc-form-field-body class="textarea">
        <textarea
          [name]="name"
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => MarvelTextareaComponent),
      multi: true,
    },
  ],
})
export class MarvelTextareaComponent implements ControlValueAccessor, OnInit {
  @Input() name: string = `marvel-textarea-${MarvelUtils.getRandomString(10)}`;
  ...
  public _id: string = `marvel-textarea-${MarvelUtils.getRandomString(10)}`;
