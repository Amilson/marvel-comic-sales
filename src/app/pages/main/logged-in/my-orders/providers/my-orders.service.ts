...
import { MyOrdersSearchModel } from './my-orders-search.model';
import { TranslateService } from '@ngx-translate/core';

@Injectable()
export class MyOrdersService extends MarvelCommonsService implements Resolve<any> {
  constructor(
  ...
  private async getData() {
    ...
    const search = new MyOrdersSearchModel({
      ...this.__search,
    });

    this.__onLoadingInProgress$.next(true);

    const docRef = this.firestore.collection('ordered_comics', (ref) => {
  ...
  setSearch(search: MyOrdersSearchModel) {
    this.__data = null;
