...
@Injectable()
export class RegisterService extends MarvelCommonsService implements Resolve<any> {
  ...
  public register(param: AuthCredentials, config: MarvelConfig) {
    const { __data } = this;
    const data = {
      ...param,
    ...
    /*this.authenticationService.register(data, config).subscribe(
      () => {},
      () => {
        this.__onLoadingInProgress$.next(false);
      }
