...
@Injectable()
export class SharedFilterCharactersService extends MarvelCommonsService implements Resolve<any> {
  private route: string = `${environment.baseUrl}/characters`;
  ...
  private mappingData(data: any[]) {
    ...
    return data.map((_: any) => {
      return new SharedFilterCharactersModel(_);
    });
  }
  ...
  getData() {
    ...
    const _search = new SharedFilterSearchModel({
      ...this.__search,
    });
